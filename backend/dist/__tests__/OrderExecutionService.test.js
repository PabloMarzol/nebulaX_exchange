...
import { OrderExecutionService, getOrderExecutionService, resetOrderExecutionService, } from '../services/hyperliquid/OrderExecutionService';
// Mock dependencies
...
describe('OrderExecutionService', () => {
    ...
    describe('Order Validation', () => {
        ...
        it('should reject order with invalid side', async () => {
            const invalidParams = { ...validOrderParams, side: 'invalid' };
            const result = await orderService.placeOrder(invalidParams);
            expect(result.success).toBe(false);
            expect(result.error).toContain('Invalid order side');
        });
        it('should reject order with invalid order type', async () => {
            const invalidParams = { ...validOrderParams, orderType: 'invalid' };
            const result = await orderService.placeOrder(invalidParams);
            expect(result.success).toBe(false);
        ...
        it('should reject order with invalid timeInForce', async () => {
            const invalidParams = { ...validOrderParams, timeInForce: 'Invalid' };
            const result = await orderService.placeOrder(invalidParams);
            expect(result.success).toBe(false);
        ...
        it('should accept valid timeInForce values', async () => {
            const validTifValues = ['Gtc', 'Ioc', 'Alo'];
    ...
    describe('Order Limits', () => {
        ...
        it('should throw error for unknown symbol', async () => {
            ...
            await expect(orderService.getOrderLimits('UNKNOWN')).rejects.toThrow('Symbol UNKNOWN not found');
        });
    });
