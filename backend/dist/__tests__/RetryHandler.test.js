...
import { createRetryHandler, retryWithBackoff, } from '../services/hyperliquid/RetryHandler';
...
describe('retryWithBackoff', () => {
    ...
    it('should accept context parameter', async () => {
        const mockFn = vi.fn().mockResolvedValue('success');
        const result = await retryWithBackoff(mockFn, { maxRetries: 2, baseDelay: 50 }, 'test-context');
        expect(result).toBe('success');
    });
    ...
    it('should fail after retries are exhausted', async () => {
        ...
        await expect(retryWithBackoff(mockFn, { maxRetries: 2, baseDelay: 50 })).rejects.toThrow('Always fails');
