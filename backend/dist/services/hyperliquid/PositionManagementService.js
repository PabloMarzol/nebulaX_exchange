...
import { hyperliquidPositions } from '../../../../shared/schema/hyperliquid.schema';
...
export class PositionManagementService extends EventEmitter {
    ...
    orderService = getOrderExecutionService();
    // Cache for position polling
    positionPollingIntervals = new Map();
    constructor() {
        super();
    ...
    async getUserPositions(userAddress) {
        try {
            ...
            const positions = userState.assetPositions.filter((pos) => {
     ...
     */
    async syncUserPositions(userId, userAddress) {
        try {
            ...
            for (const apiPos of apiPositions) {
                ...
                const [existingPosition] = await db
                    ...
                    .where(and(eq(hyperliquidPositions.userId, userId), eq(hyperliquidPositions.symbol, coin), isNull(hyperliquidPositions.closedAt)))
                    .limit(1);
                if (existingPosition) {
     ...
     */
    async getUserPositionsFromDb(userId, includeClosedPositions = false) {
        try {
    ...
    async getPositionBySymbol(userId, symbol) {
        try {
            const [position] = await db
                .select()
                .from(hyperliquidPositions)
                .where(and(eq(hyperliquidPositions.userId, userId), eq(hyperliquidPositions.symbol, symbol), isNull(hyperliquidPositions.closedAt)))
                .limit(1);
    ...
    async closePosition(params) {
        try {
            ...
            console.log(`[PositionManagementService] Closing ${position.side} position: ${symbol} (size: ${positionSize})`);
     ...
     */
    async startPositionPolling(userId, userAddress, intervalMs = 5000) {
     ...
     */
    async getMarginSummary(userAddress) {
        try {
            const userState = await this.client.getUserState(userAddress);
            if (!userState) {
                return null;
 ...
 * Singleton instance
 */
let positionManagementServiceInstance = null;
/**
 * Get the singleton instance of PositionManagementService
 */
export function getPositionManagementService() {
 ...
 */
export function resetPositionManagementService() {
    if (positionManagementServiceInstance) {
