import { InfoClient, ExchangeClient, SubscriptionClient, HttpTransport, WebSocketTransport, } from '@nktkas/hyperliquid';
/**
...
export class HyperliquidClient {
    ...
    async subscribeToOrderbook(symbol, callback) {
        try {
            const subscription = await this.subscriptionClient.l2Book({ coin: symbol }, (event) => {
                callback(event);
            });
    ...
    async subscribeToTrades(symbol, callback) {
        try {
            const subscription = await this.subscriptionClient.trades({ coin: symbol }, (event) => {
                callback(event);
            });
     ...
     */
    async subscribeToUserEvents(userAddress, callback) {
        try {
            const subscription = await this.subscriptionClient.userEvents({ user: userAddress }, (event) => {
                callback(event);
            });
     ...
     */
    async subscribeToCandles(symbol, interval, callback) {
        try {
            const subscription = await this.subscriptionClient.candle({ coin: symbol, interval }, (event) => {
                callback(event);
            });
 ...
 */
let hyperliquidClientInstance = null;
/**
...
export function getHyperliquidClient() {
    if (!hyperliquidClientInstance) {
        ...
        if (!apiPrivateKey) {
            throw new Error('HYPERLIQUID_API_PRIVATE_KEY or HYPERLIQUID_LIVE_API_PRIVATE_KEY is required in environment variables');
        }
