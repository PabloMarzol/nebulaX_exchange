...
import { hyperliquidOrders, hyperliquidFills } from '../../../../shared/schema/hyperliquid.schema';
...
export class OrderStatusService extends EventEmitter {
    ...
    subscriptions = new Map(); // userAddress -> subscription
    constructor() {
        super();
     ...
     */
    async subscribeToUserOrders(userId, userAddress) {
        ...
        try {
            ...
            const subscription = await this.client.subscribeToUserEvents(userAddress, (data) => {
     ...
     */
    async unsubscribeFromUserOrders(userAddress) {
     ...
     */
    async handleUserEvent(userId, userAddress, event) {
        try {
     ...
     */
    async handleFillEvent(userId, userAddress, fill) {
        try {
            const { oid, // Hyperliquid order ID
            coin, // Symbol
            ...
            const newAvgPrice = currentFilledSize > 0
            ...
            console.log(`[OrderStatusService] Order ${order.internalOrderId} updated: ${order.status} -> ${newStatus} (filled: ${newFilledSize}/${totalSize})`);
            // Emit fill event
            this.emit('orderFill', {
                userId,
 ...
 * Singleton instance
 */
let orderStatusServiceInstance = null;
/**
 * Get the singleton instance of OrderStatusService
 */
export function getOrderStatusService() {
 ...
 */
export function resetOrderStatusService() {
    if (orderStatusServiceInstance) {
