...
import { hyperliquidOrders, hyperliquidPositions, hyperliquidReconciliations, } from '../../../../shared/schema/hyperliquid.schema';
...
export class ReconciliationService extends EventEmitter {
     ...
     */
    async reconcileUserOrders(userId, userAddress) {
        const discrepancies = [];
        try {
            ...
            const dbOrders = await db
                ...
                .where(and(eq(hyperliquidOrders.userId, userId), inArray(hyperliquidOrders.status, ['pending', 'open', 'partially_filled'])));
            ...
            const apiOrders = userState?.assetPositions
                ?.flatMap((asset) => asset.position?.openOrders || [])
            ...
            for (const dbOrder of dbOrders) {
                ...
                if (!apiOrder) {
                    ...
                    console.warn(`[ReconciliationService] Discrepancy: Order ${dbOrder.internalOrderId} ` +
                    ...
                    console.log(`[ReconciliationService] Auto-resolving: Marking order ${dbOrder.internalOrderId} as filled`);
                    await db
            ...
            for (const apiOrder of apiOrders) {
                ...
                if (!dbOrder) {
                    console.error(`[ReconciliationService] Critical: Order ${apiOrder.oid} on Hyperliquid but not in DB!`);
                    const discrepancy = {
                        type: 'order',
     ...
     */
    async reconcileUserPositions(userId, userAddress) {
        const discrepancies = [];
        try {
            ...
            const userState = await this.client.getUserState(userAddress);
            const apiPositions = userState?.assetPositions?.filter((asset) => {
            ...
            for (const dbPos of dbPositions) {
                ...
                if (!apiPos) {
                    ...
                    console.warn(`[ReconciliationService] Discrepancy: Position ${dbPos.symbol} ` +
                    ...
                    console.log(`[ReconciliationService] Auto-resolving: Marking position ${dbPos.symbol} as closed`);
                    await db
                    ...
                    if (sizeDiff > dbSize * 0.001) {
                        console.warn(`[ReconciliationService] Position size mismatch: ${dbPos.symbol} ` +
            ...
            for (const apiPos of apiPositions) {
                ...
                if (!dbPos) {
                    console.warn(`[ReconciliationService] Position ${apiPos.position.coin} on Hyperliquid but not in DB`);
                    const discrepancy = {
                        type: 'position',
     ...
     */
    async reconcileUser(userId, userAddress) {
        ...
        try {
            ...
            console.log(`[ReconciliationService] Reconciliation complete: ${result.ordersChecked} orders, ` +
     ...
     */
    startPeriodicReconciliation(userId, userAddress, intervalMs = 300000 // 5 minutes
    ) {
 ...
 */
export function resetReconciliationService() {
    if (reconciliationServiceInstance) {
