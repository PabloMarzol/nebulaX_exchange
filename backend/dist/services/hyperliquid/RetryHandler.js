    ...
    async execute(fn, context) {
        ...
        for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
            ...
            if (this.config.timeout && Date.now() - startTime > this.config.timeout) {
                const timeoutError = new Error(`Operation timed out after ${this.config.timeout}ms (${attempt} attempts)`);
            ...
            try {
                ...
                if (attempt > 0) {
                    console.log(`[RetryHandler] ${context || 'Operation'} succeeded after ${attempt} retries`);
                }
                return result;
            }
            catch (error) {
                ...
                if (!isRetryableError(error, this.config.retryableErrors)) {
                    console.log(`[RetryHandler] ${context || 'Operation'} failed with non-retryable error:`, error.message);
                    throw error;
                }
                // Check if we've exhausted retries
                if (attempt >= this.config.maxRetries) {
                    console.log(`[RetryHandler] ${context || 'Operation'} failed after ${this.config.maxRetries} retries:`, error.message);
                    throw error;
                }
                // Calculate delay and wait
                const delay = calculateDelay(attempt, this.config);
                console.log(`[RetryHandler] ${context || 'Operation'} failed (attempt ${attempt + 1}/${this.config.maxRetries + 1}). Retrying in ${delay}ms... Error: ${error.message}`);
                this.totalRetries++;
                await sleep(delay);
            }
        }
 ...
 * );
 */
export async function retryWithBackoff(fn, config, context) {
