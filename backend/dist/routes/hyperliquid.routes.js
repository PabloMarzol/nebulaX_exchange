 ...
 */
router.get('/user/:address/orders', async (req, res) => {
    try {
        const { address } = req.params;
        // Validate address format
        if (!address.match(/^0x[a-fA-F0-9]{40}$/)) {
...
router.post('/orders', async (req, res) => {
    try {
        const { userId, userAddress, symbol, side, orderType, price, size, timeInForce, reduceOnly, postOnly, clientOrderId, } = req.body;
...
router.get('/orders/:userId', async (req, res) => {
    try {
        ...
        const orders = await orderExecutionService.getUserOrders(userId, limit ? parseInt(limit) : 100);
        res.json({
            success: true,
            data: orders,
            count: orders.length,
 ...
 */
router.get('/orders/:userId/open', async (req, res) => {
    try {
        const { userId } = req.params;
 ...
 */
router.delete('/orders/:userId/all', async (req, res) => {
    try {
        const { userId } = req.params;
 ...
 */
router.get('/orders/limits/:symbol', async (req, res) => {
    try {
        const { symbol } = req.params;
 ...
 * Estimate order fees
 */
router.post('/orders/fees/estimate', (req, res) => {
    try {
...
router.get('/positions/:userId', async (req, res) => {
    try {
        ...
        const positions = await positionService.getUserPositionsFromDb(userId, includeClosed === 'true');
        res.json({
            success: true,
            data: positions,
 ...
 */
router.get('/positions/:userId/:symbol', async (req, res) => {
    try {
 ...
 */
router.get('/margin/:userAddress', async (req, res) => {
    try {
 ...
 */
router.post('/positions/polling/start', async (req, res) => {
    try {
 ...
 */
router.post('/positions/polling/stop', async (req, res) => {
    try {
 ...
 */
router.post('/reconciliation/start', async (req, res) => {
    try {
        const { userId, userAddress, intervalMs } = req.body;
        if (!userId || !userAddress) {
            return res.status(400).json({
 ...
 */
router.post('/reconciliation/stop', (req, res) => {
    try {
