 ...
 */
export function setupHyperliquidSocket(io) {
    ...
    orderStatusService.on('orderStatusChange', (statusEvent) => {
        ...
        console.log(`[HyperliquidSocket] Order status change emitted: ${statusEvent.orderId} (${statusEvent.previousStatus} -> ${statusEvent.newStatus})`);
    });
    ...
    positionService.on('positionUpdate', (positionEvent) => {
        ...
        console.log(`[HyperliquidSocket] Position update emitted: ${positionEvent.symbol} (${positionEvent.side}) PnL: ${positionEvent.unrealizedPnl}`);
    });
    ...
    positionService.on('positionClosed', (closedEvent) => {
        ...
        console.log(`[HyperliquidSocket] Position closed emitted: ${closedEvent.symbol} (${closedEvent.side}) Realized PnL: ${closedEvent.realizedPnl}`);
    });
    ...
    io.on('connection', (socket) => {
         ...
         */
        socket.on('subscribe:candles', async ({ symbol, interval }) => {
            if (!symbol || !interval) {
         ...
         */
        socket.on('subscribe:userEvents', async (userAddress) => {
         ...
         */
        socket.on('subscribe:orders', async ({ userId, userAddress }) => {
         ...
         */
        socket.on('subscribe:positions', async ({ userId, userAddress, intervalMs }) => {
            if (!userId || !userAddress) {
                socket.emit('error', { message: 'Invalid userId or userAddress' });
                return;
            }
         ...
         */
        socket.on('unsubscribe:orderbook', async (symbol) => {
            try {
         ...
         */
        socket.on('unsubscribe:candles', async ({ symbol, interval }) => {
            try {
         ...
         */
        socket.on('unsubscribe:userEvents', async (userAddress) => {
            try {
         ...
         */
        socket.on('unsubscribe:orders', async ({ userId, userAddress }) => {
            try {
         ...
         */
        socket.on('unsubscribe:positions', async ({ userId }) => {
            try {
