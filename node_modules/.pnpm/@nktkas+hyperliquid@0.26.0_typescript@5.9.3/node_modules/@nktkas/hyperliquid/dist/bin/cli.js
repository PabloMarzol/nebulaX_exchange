...
import { parseArgs, transformArgs } from "./_utils.js";
...
class EchoTransport {
    ...
    request(_, payload) {
        return new Promise((resolve) => resolve({ status: "ok", response: payload }));
    }
}
async function executeEndpointMethod(endpoint, method, args) {
    ...
    if (endpoint === "info") {
        ...
        const pk = v.parse(v.pipe(v.string(), v.hexadecimal()), args["private-key"], { message: 'Invalid format "private-key": Expected 32-byte hexadecimal string' });
        delete args["private-key"]; // just in case
        const defaultVaultAddress = v.parse(v.optional(v.pipe(v.string(), v.hexadecimal())), args.vault, { message: 'Invalid format "vault": Expected 20-byte hexadecimal string OR nothing' });
