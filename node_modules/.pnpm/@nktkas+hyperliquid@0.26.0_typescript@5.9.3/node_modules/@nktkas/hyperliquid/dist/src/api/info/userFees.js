...
import { Address, Decimal, parser, UnsignedDecimal } from "../_base.js";
// -------------------- Schemas --------------------
/**
 * Request user fees.
...
export const UserFeesResponse = /* @__PURE__ */ (() => {
    return v.pipe(v.object({
        ...
        v.pipe(v.object({
            ...
            userCross: v.pipe(UnsignedDecimal, v.description("User cross-trade volume.")),
            /** User add-liquidity volume. */
            userAdd: v.pipe(UnsignedDecimal, v.description("User add-liquidity volume.")),
        ...
        feeSchedule: v.pipe(v.object({
            ...
            add: v.pipe(UnsignedDecimal, v.description("Add-liquidity fee rate.")),
            /** Spot cross-trade fee rate. */
            spotCross: v.pipe(UnsignedDecimal, v.description("Spot cross-trade fee rate.")),
        ...
        userCrossRate: v.pipe(UnsignedDecimal, v.description("User cross-trade rate.")),
        /** User add-liquidity rate. */
        userAddRate: v.pipe(UnsignedDecimal, v.description("User add-liquidity rate.")),
        /** User spot cross-trade rate. */
        userSpotCrossRate: v.pipe(UnsignedDecimal, v.description("User spot cross-trade rate.")),
        /** User spot add-liquidity rate. */
        userSpotAddRate: v.pipe(UnsignedDecimal, v.description("User spot add-liquidity rate.")),
        /** Active referral discount rate. */
        activeReferralDiscount: v.pipe(UnsignedDecimal, v.description("Active referral discount rate.")),
        /** Trial details. */
        trial: v.pipe(v.nullable(v.unknown()), v.description("Trial details.")),
        /** Fee trial escrow amount. */
        feeTrialEscrow: v.pipe(UnsignedDecimal, v.description("Fee trial escrow amount.")),
        ...
        stakingLink: v.pipe(v.nullable(v.object({
             ...
             */
            stakingUser: v.pipe(Address, v.description("Linked account address:" +
                "\n- When queried by staking account: contains trading account address." +
                "\n- When queried by trading account: contains staking account address.")),
            /**
             * Link status:
            ...
            ]), v.description("Link status:" +
                "\n- `requested` = link initiated by trading user, awaiting staking user confirmation" +
                "\n- `stakingUser` = response queried by staking account" +
                "\n- `tradingUser` = response queried by trading account")),
        })), v.description("Permanent link between staking and trading accounts. Staking user gains full control of trading account funds. Staking user forfeits own fee discounts.")),
